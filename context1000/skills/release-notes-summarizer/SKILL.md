...
```bash
/context1000:guide "Migrate from <old> to <new>" --audience <affected-teams>
```

### When New Feature Lacks RULE

```bash
/context1000:rule "<Feature> Usage Standards" --severity warn --scope <area>
```

### When Architectural Change Lacks ADR

```bash
/context1000:adr "Record Decision: <Change>" --status accepted --template madr --scope <area>
```
...
/context1000:guide "Migrate Database Connection Pool Config" --audience backend
/context1000:rule "Database Connection Pool Standards" --severity required --scope backend
