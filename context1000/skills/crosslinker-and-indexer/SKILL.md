...
```bash
/context1000:adr "Record Decision from RFC-<ID>: <Title>" --status accepted --template madr --scope <area>
```
...
```bash
/context1000:rule "<Standard from ADR>" --severity required --scope <area>
```
...
2. If not: /context1000:adr "Record Decision from RFC-0023: Adopt Kafka" --status accepted --template madr --scope platform
...
2. If not: /context1000:rule "Kafka Usage Standards" --severity required --scope platform
...
2. If not: /context1000:guide "Configure Kafka Topics" --audience backend
...
3. Propose cross-links:
   ...
   c) Recommend RULE creation:
      /context1000:rule "API Gateway Usage Standards" --severity required --scope infrastructure

   d) Recommend GUIDE creation:
      /context1000:guide "Configure Kong API Gateway" --audience infra
