  ...
  try {
    ...
    if (transcriptPath && fs.existsSync(transcriptPath)) {
      try {
        ...
        isInsideSlashCommand =
          ...
          recentContext.includes("/context1000:guide") ||
          recentContext.includes("/context1000:adr") ||
          recentContext.includes("/context1000:rfc") ||
          recentContext.includes("/context1000:rule") ||
          recentContext.includes("/context1000:project");
    ...
    if (toolName === "Write") {
      ...
      if (filePath.includes(".context1000")) {
        blockTool(
          toolName,
          `🚫 BLOCKED: Cannot use Write tool to create .context1000 documentation directly.

You must use SlashCommand tool to invoke:
  /context1000:project "<projectName>"
  /context1000:guide "<topic>" [--project <projectName>]
  /context1000:rule "<name>" [--project <projectName>]
  /context1000:adr "<title>" [--project <projectName>]
  /context1000:rfc "<title>" [--project <projectName>]

Example:
  SlashCommand tool with command: "/context1000:guide \\"InstantDB integration\\""
  SlashCommand tool with command: "/context1000:adr \\"Choose database\\" --project user-service"
...
The slash command will create the file in the correct location with correct structure.`
    ...
    if (toolName === "Edit") {
      ...
      if (filePath.includes(".context1000") && !filePath.includes("context1000-documentation-writer")) {
        blockTool(
          toolName,
          `🚫 BLOCKED: Cannot use Edit tool on .context1000 documentation.
...
Use SlashCommand tool to CREATE new documents via /context1000:* commands.`
        );
        return;
      }
    }
    ...
    if (toolName === "Bash") {
      ...
      for (const { pattern, desc } of dangerousPatterns) {
        if (pattern.test(command)) {
          blockTool(
            toolName,
            `🚫 BLOCKED: Cannot use Bash for file/directory creation in .context1000/.

Blocked command: ${command}
Reason: ${desc}

You must use SlashCommand tool with /context1000:* commands to create documentation.
